...
void TIMER0_IRQHandler()
{
    ...
    else
    {
        if(SERVO_INDEX(_timer1,Channel[_timer1]) < ServoCount && SERVO(_timer1,Channel[_timer1]).Pin.isActive == true)
        {
            ...
            digitalWrite(SERVO(_timer1,Channel[_timer1]).Pin.nbr, LOW);
        }
    }
    ...
    {
        NRF_TIMER0->CC[0] += SERVO(_timer1,Channel[_timer1]).ticks;
        ...
        if(SERVO(_timer1,Channel[_timer1]).Pin.isActive == true)
        {
            ...
            digitalWrite(SERVO(_timer1,Channel[_timer1]).Pin.nbr, HIGH);
        }
    }
    else
    {
        // allow a few ticks to ensure the next OCR1A not missed
        if( NRF_TIMER0->CC[0] +4 < usToTicks(REFRESH_INTERVAL) )
        {
            NRF_TIMER0->CC[0] = (unsigned int)usToTicks(REFRESH_INTERVAL);
        }
        else
        {
            NRF_TIMER0->CC[0] += 4;
...
static void InitISR_handler()
{
    ...
    NRF_TIMER0->PRESCALER = 3;
    NRF_TIMER0->BITMODE = TIMER_BITMODE_BITMODE_16Bit;
    NRF_TIMER0->TASKS_CLEAR = 1;
    NRF_TIMER0->CC[0] = (uint32_t)(VARIANT_MCK/8/1000); // 1ms
    NRF_TIMER0->EVENTS_COMPARE[0] = 0;
...
uint8_t Servo::attach(uint32_t pin, uint16_t min, uint16_t max)
{
    ...
    if(this->servoIndex < MAX_SERVOS)
    {
        ...
        servos[this->servoIndex].Pin.nbr = pin;//nrf_pin;
